    ...
    methods: [
      {
        methodId: "produce",
        displayName: "Produce",
        inputPorts: [atmPort("start", false)],
...
`,
  ...
  const flow = {
    ...
    triggers: [
      { triggerId: "manual", kind: "manual" },
      { triggerId: "required", kind: "manual" },
    ],
    triggerBindings: [
      { triggerId: "manual", targetNodeId: "produce", targetPortId: "start" },
      { triggerId: "required", targetNodeId: "consume", targetPortId: "middle" },
    ],
...
test("a bodyless trigger is a scheduler signal, not an untyped ABI frame", async () => {
  ...
  const current =
    exports.space_data_module_runtime_get_current_invocation_descriptor ??
    exports._space_data_module_runtime_get_current_invocation_descriptor;
  const complete =
  ...
  assert.equal(begin(0, 64) | 0, 0, "bodyless wakeup must consume zero frames");
  const descriptor = current() >>> 0;
  assert.notEqual(descriptor, 0);
  const view = new DataView(host.memory.buffer);
  assert.equal(view.getUint32(descriptor + 16, true), 0, "frames pointer must be null");
  assert.equal(view.getUint32(descriptor + 20, true), 0, "frame count must be zero");
  complete(0);
  assert.equal(ready() >>> 0, FLOW_INVALID_INDEX);

  host.enqueueTrigger(1);
  assert.equal(
    ready() >>> 0,
    FLOW_INVALID_INDEX,
    "a bodyless signal cannot satisfy a required data port",
  );
  assert.equal(host.getIngressState(1).totalDropped, 1n);
  assert.equal(host.getIngressState(1).queuedFrames, 0);
});
