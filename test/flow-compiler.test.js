...
test("canonical flow PLG binds isomorphic dispatch and paired edge fallback policy", () => {
  const producer = {
    ...producerDependency,
    guestLink: null,
  };
  ...
  assert.equal(manifest.flowNodes[0].dispatchModel, "isomorphic");
  assert.equal(manifest.flowNodes[1].dispatchModel, "isomorphic");
  assert.deepEqual(
    {
      canonicalWireFormat: manifest.flowEdges[0].contract?.canonicalType?.wireFormat,
      alignedWireFormat: manifest.flowEdges[0].contract?.alignedType?.wireFormat,
      canonicalFallbackAvailable:
        manifest.flowEdges[0].contract?.canonicalFallbackAvailable,
      alignedEligible: manifest.flowEdges[0].contract?.alignedEligible,
      routePolicy: manifest.flowEdges[0].contract?.routePolicy,
    },
    {
  ...
  assert.equal(decoded.flowNodes[0].dispatchModel, "isomorphic");
  assert.equal(decoded.flowNodes[1].dispatchModel, "isomorphic");
  assert.equal(decoded.flowEdges[0].contract.canonicalType.wireFormat, "flatbuffer");
  assert.equal(decoded.flowEdges[0].contract.alignedType.wireFormat, "aligned-binary");
  assert.equal(decoded.flowEdges[0].contract.canonicalFallbackAvailable, true);
  assert.equal(decoded.flowEdges[0].contract.alignedEligible, false);
  assert.equal(decoded.flowEdges[0].contract.routePolicy, "canonical-only");
});
...
test("root-manifest capabilities remain the conservative fallback", () => {
  ...
  assert.equal(check.ok, true, JSON.stringify(check.issues));
  assert.deepEqual(check.capabilities, ["http", "wallet_sign"]);
});
...
test("flow compile binds exact edge contracts into the emitted runtime WASM", async () => {
  ...
  const compilation = await compileModuleFromSource({
    ...
    // Explicit since browser targeting no longer implies a toolchain: these
    // fixtures previously relied on the removed browser -> Emscripten inference.
    threadModel: "single-thread",
    sourceCode: `
#include <stdint.h>
...
`,
  ...
  // Change only the internal edge contract. Both the canonical flow PLG and
  // compiled runtime must bind the changed schema identity and aligned layout.
  ...
  const originalFlowPlg = decodePluginManifest(result.flowPlgBytes);
  const changedFlowPlg = decodePluginManifest(changedResult.flowPlgBytes);
  const originalContract = originalFlowPlg.flowEdges[0].contract;
  const changedContract = changedFlowPlg.flowEdges[0].contract;
  assert.deepEqual(
    {
      schemaVersion: originalContract.alignedType.schemaVersion,
      schemaHash: [...originalContract.alignedType.schemaHash],
      byteLength: originalContract.alignedType.byteLength,
      fixedStringLength: originalContract.alignedType.fixedStringLength,
      requiredAlignment: originalContract.alignedType.requiredAlignment,
      canonicalFallbackAvailable: originalContract.canonicalFallbackAvailable,
      alignedEligible: originalContract.alignedEligible,
      routePolicy: originalContract.routePolicy,
    },
    {
      schemaVersion: "7.1.2",
      schemaHash: [0xde, 0xad, 0xbe, 0xef],
  ...
  assert.deepEqual(
    {
      schemaVersion: changedContract.alignedType.schemaVersion,
      schemaHash: [...changedContract.alignedType.schemaHash],
      byteLength: changedContract.alignedType.byteLength,
      fixedStringLength: changedContract.alignedType.fixedStringLength,
      requiredAlignment: changedContract.alignedType.requiredAlignment,
      canonicalFallbackAvailable: changedContract.canonicalFallbackAvailable,
      alignedEligible: changedContract.alignedEligible,
      routePolicy: changedContract.routePolicy,
    },
    {
      schemaVersion: "7.1.3",
      schemaHash: [0xde, 0xad, 0xfa, 0xce],
