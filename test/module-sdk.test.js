...
test("source compilation can emit an imported shared-memory browser artifact", async (t) => {
  ...
  const result = await compileModuleFromSource({
    ...
    language: "c",
    // Explicit: this test is specifically about the LEGACY Emscripten
    // shared-memory browser artifact. Browser targeting no longer implies a
    // toolchain, so the Emscripten path must now be asked for by name.
