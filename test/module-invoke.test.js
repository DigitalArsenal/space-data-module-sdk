...
const OPTIONAL_SIGNAL_SOURCE = `#include <stdint.h>
#include "space_data_module_invoke.h"

int signal(void) {
  if (plugin_get_input_count() != 0u) {
    plugin_set_error("signal-frame-leaked", "An opaque wakeup token reached the typed guest input list.");
    return 92;
  }
  return 0;
}
...
test("an exact empty untyped host wakeup invokes a method through an optional paired port", async () => {
  const manifest = createInvokeManifest({
    pluginId: "com.digitalarsenal.examples.invoke-optional-host-wakeup",
    invokeSurfaces: ["direct"],
    methodId: "signal",
    inputPortIds: ["wakeup"],
    outputPortIds: [],
  });
  manifest.methods[0].inputPorts[0].required = false;
  manifest.methods[0].inputPorts[0].minStreams = 0;
  const compilation = await compileModuleFromSource({
    manifest,
    sourceCode: OPTIONAL_SIGNAL_SOURCE,
    language: "c",
  });

  try {
    const { instance } = instantiateWithWasi(compilation.wasmBytes);
    const wakeup = invokeDirect(
      instance,
      encodePluginInvokeRequest({
        methodId: "signal",
        inputs: [
    ...
    assert.equal(wakeup.statusCode, 0, wakeup.errorMessage);

    const nonemptyUntyped = invokeDirect(
      instance,
      encodePluginInvokeRequest({
        methodId: "signal",
        inputs: [
    ...
    assert.equal(nonemptyUntyped.statusCode, 400);
    assert.equal(nonemptyUntyped.errorCode, "unsupported-input-type");
  } finally {
    await cleanupCompilation(compilation);
  }
});
...
test("generated guest validation matches exact SDS schema identity before dispatch", async () => {
  ...
  try {
    ...
    const canonicalWithPairedLayoutHints = invokeDirect(
      instance,
    ...
    assert.equal(canonicalWithPairedLayoutHints.statusCode, 91);
    assert.equal(canonicalWithPairedLayoutHints.errorCode, "handler-executed");
