...
function resolveFlowEdgeRoutePolicy(value) {
  ...
  return (
    flowEdgeRoutePolicyByName[canonicalEnumKey(value)] ??
...
function addFlowEdgeContract(builder, contract = {}) {
  ...
  const alignedTypeOffset = alignedType
    ...
    : 0;
  const canonicalFallbackAvailable = Boolean(
    pick(
      contract,
  ...
  const routePolicy = resolveFlowEdgeRoutePolicy(
  ...
  PLGFlowEdgeContract.addAlignedEligible(builder, alignedEligible);
  PLGFlowEdgeContract.addRoutePolicy(builder, routePolicy);
  return PLGFlowEdgeContract.endPLGFlowEdgeContract(builder);
}

function flowEdgeContractToObject(contract) {
  if (!contract) {
    throw new TypeError("Decoded PLG flow edge is missing its required contract.");
  }
  return {
    canonicalType: typeRefToObject(contract.CANONICAL_TYPE()),
    ...(contract.ALIGNED_TYPE()
      ? { alignedType: typeRefToObject(contract.ALIGNED_TYPE()) }
      : {}),
...
function addFlowEdge(builder, edge = {}) {
  ...
  if (!contract) {
    throw new TypeError("PLG flow edge requires contract.");
  }
  const contractOffset = addFlowEdgeContract(builder, contract);
  ...
  return PLGFlowEdge.endPLGFlowEdge(
    builder,
  );
}
...
function flowEdgeToObject(edge) {
  return {
    ...
    contract: flowEdgeContractToObject(edge.CONTRACT()),
  };
}
