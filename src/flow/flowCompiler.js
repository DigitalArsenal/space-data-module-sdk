...
function normalizeGuestThreadModel(value) {
  ...
  if (
    ...
    // below and be linked by emcc — exactly the toolchain drift this model
    // exists to prevent. The per-guest sequential guarantee is enforced where
    // it belongs: on each MODULE artifact, by assertSequentialArtifact.
...
export function checkFlowProgram({ flow, dependencies = new Map() } = {}) {
  ...
  if (flow?.linkedStore !== undefined) {
    try {
      linkedStore = validateFlatsqlLinkedStoreDescriptor(flow.linkedStore);
    } catch (error) {
      pushIssue(
        issues,
        "error",
        "invalid-linked-store",
        error.message,
        "flow.linkedStore",
      );
    }
  ...
  if (!linkedStore) {
    ...
    for (const entry of resolvedNodes) {
      if (entry.dispatchModel !== "linked-direct") continue;
      const guestLink = selectedGuestLink(
        entry.dependency,
        Boolean(engineLinkage),
      );
      try {
        if (
          listFlatsqlPersistenceImports(guestLink?.objectBytes ?? []).length > 0
        ) {
          persistenceWriters.push(entry.node.pluginId);
        }
      } catch {
        // A malformed guest object is rejected by the linker. Flow check keeps
        // its existing lightweight fixture support and only classifies valid
        // wasm objects here.
      }
    }
    if (persistenceWriters.length > 0) {
      pushIssue(
        issues,
        "error",
        "missing-linked-store",
        `flow.linkedStore is required because linked plugin(s) ${[
          ...new Set(persistenceWriters),
        ].join(", ")} import flatsql.ingest_record.`,
        "flow.linkedStore",
      );
    }
  }
...
function manifestPayloadTypeRef(typeRef, wireFormat) {
  if (!typeRef) return null;
  const schemaHash =
    typeRef.schemaHash instanceof Uint8Array || Array.isArray(typeRef.schemaHash)
      ? Array.from(typeRef.schemaHash)
      : typeRef.schemaHash;
  return {
    schemaName: typeRef.schemaName ?? null,
    fileIdentifier: typeRef.fileIdentifier ?? null,
    schemaVersion: typeRef.schemaVersion ?? null,
    ...(schemaHash === undefined || schemaHash === null ? {} : { schemaHash }),
...
function manifestEdgeContract(contract, fromDispatchModel, toDispatchModel) {
  const canonicalType =
    contract?.canonical?.producer ?? contract?.canonical?.consumer ?? null;
  if (!canonicalType) {
    throw new TypeError("Validated edge contract has no canonical fallback type.");
  }
  const alignedType =
    contract?.aligned?.producer ?? contract?.aligned?.consumer ?? null;
  const canonicalFallbackAvailable =
    contract.compatibleWireFormats?.includes("flatbuffer") === true;
  const alignedCompatible =
  ...
  return {
    canonicalType: manifestPayloadTypeRef(canonicalType, "flatbuffer"),
    ...(alignedCompatible
      ? { alignedType: manifestPayloadTypeRef(alignedType, "aligned-binary") }
      : {}),
...
export function buildFlowModuleManifest({ flow, check, dependencies }) {
  ...
  return {
      ...
      return {
        ...
        dispatchModel:
          checkedNode?.dispatchModel === "host"
            ? "host-capability"
            : checkedNode?.dispatchModel ?? null,
        ...(config ? { config } : {}),
    ...
    flowEdges: edges.map((edge, index) => {
      const fromDispatchModel = checkedNodeById.get(
        edge.fromNodeId,
      )?.dispatchModel;
      const toDispatchModel = checkedNodeById.get(edge.toNodeId)?.dispatchModel;
      return {
...
export function generateFlowTables({ flow, check, dependencies }) {
  ...
  if (edgeDescriptors.some((descriptor) => descriptor.schemaHash.length > 0)) {
    lines.push("");
  }
...
export async function compileFlowProgram({
  ...
  let linkedStoreDescriptorSha256 = null;
  if (check.linkedStore) {
    const descriptorBytes = encodeFlatsqlLinkedStoreDescriptor(
      check.linkedStore,
    );
    linkedStoreDescriptorSha256 = bytesToHex(
      await sha256Bytes(descriptorBytes),
    );
    wasmBytes = appendWasmCustomSection(
      wasmBytes,
      FLATSQL_LINKED_STORE_SECTION_NAME,
  ...
  assertFlatsqlLinkedStoreDescriptorForWasm(wasmBytes, check.linkedStore);
  ...
  const artifact = {
    ...
    ...(linkedStoreDescriptorSha256
      ? {
          linkedStoreDescriptor: {
            sectionName: FLATSQL_LINKED_STORE_SECTION_NAME,
            sha256: linkedStoreDescriptorSha256,
          },
        }
      : {}),
