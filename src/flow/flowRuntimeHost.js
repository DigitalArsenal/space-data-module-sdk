...
function typeRefMatchesEdge(typeRef = {}, edge, wireFormat) {
  if (typeRef.schemaName && typeRef.schemaName !== edge.schemaName) return false;
  if (typeRef.fileIdentifier && typeRef.fileIdentifier !== edge.fileIdentifier) return false;
  if (typeRef.rootTypeName && typeRef.rootTypeName !== edge.rootTypeName) return false;
  if (typeRef.schemaVersion && typeRef.schemaVersion !== edge.schemaVersion) return false;
  ...
  if (schemaHashInput !== undefined && schemaHashInput !== null) {
    ...
    if (schemaHash?.length > 0 && !bytesEqual(schemaHash, edge.schemaHash)) return false;
  }

  if (wireFormat === "aligned-binary") {
    if (!edge.alignedEligible) return false;
    if (Number(typeRef.byteLength ?? 0) !== edge.alignedByteLength) return false;
    if (Number(typeRef.requiredAlignment ?? 0) !== edge.alignedRequiredAlignment) {
      return false;
    }
    if (
      typeRef.fixedStringLength !== undefined &&
      Number(typeRef.fixedStringLength) !== edge.alignedFixedStringLength
    ) {
      return false;
    }
  }
...
export async function createFlowRuntimeHost(options = {}) {
  ...
  const host = {
    ...
    getEdgeDescriptor(index) {
      if (!Number.isInteger(index) || index < 0 || index >= this.edgeCount) {
    ...
    enqueueTriggerFrame(triggerIndex, frame = {}) {
      ...
      try {
        ...
        for (let index = 0; index < this.edgeCount; index += 1) {
          const edge = this.getEdgeDescriptor(index);
          if (typeRefMatchesEdge(typeRef, edge, wireFormat)) {
            typeDescriptorIdx = index;
            break;
          }
        }
        if (
          typeDescriptorIdx === FLOW_INVALID_INDEX &&
          this.edgeCount > 0 &&
          !typeRef.schemaName
        ) {
          typeDescriptorIdx = 0;
        }
        if (typeDescriptorIdx === FLOW_INVALID_INDEX && this.edgeCount > 0) {
    ...
    async drain(handlers = {}, options = {}) {
      ...
      for (let i = 0; i < maxIterations; i++) {
        ...
        for (let f = 0; f < inv.frameCount; f++) {
          ...
          const edge =
            fd.typeDescriptorIdx < this.edgeCount
              ? this.getEdgeDescriptor(fd.typeDescriptorIdx)
              : null;
          const wireFormat = fd.wireFormat === 1 ? "aligned-binary" : "flatbuffer";
          const typeRef = edge
            ? {
                schemaName: edge.schemaName,
                fileIdentifier: edge.fileIdentifier,
                ...(edge.schemaVersion ? { schemaVersion: edge.schemaVersion } : {}),
                ...(edge.schemaHash.length > 0 ? { schemaHash: edge.schemaHash } : {}),
                rootTypeName: edge.rootTypeName,
                wireFormat,
                ...(wireFormat === "aligned-binary"
                  ? {
                      byteLength: edge.alignedByteLength,
                      ...(edge.alignedFixedStringLength > 0
                        ? { fixedStringLength: edge.alignedFixedStringLength }
                        : {}),
                      requiredAlignment: edge.alignedRequiredAlignment,
                    }
                  : {}),
              }
            : null;
          ...
          if (outputs.length > 0) {
            ...
            outputs.forEach((out, idx) => {
              ...
              for (let edgeIndex = 0; edgeIndex < this.edgeCount; edgeIndex += 1) {
                ...
                if (!typeRefMatchesEdge(outputTypeRef, edge, outputWireFormat)) continue;
