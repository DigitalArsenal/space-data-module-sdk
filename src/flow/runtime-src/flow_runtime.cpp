...
static uint32_t g_pending_bodyless_triggers[FLOW_NODE_COUNT];
...
static bool flow_binding_requires_frame(const FlowTriggerBinding &binding) {
  for (uint32_t r = 0; r < FLOW_REQUIRED_PORT_COUNT; r++) {
    if (g_required_ports[r].node_index == binding.target_node &&
        strcmp(g_required_ports[r].port_id, binding.port) == 0) {
      return true;
    }
  }
  return false;
...
// Readiness: a node is ready when it has queued data or a bodyless scheduler
// signal, AND every required input port of its bound method (compiled in from
// the dependency manifest) has at least one queued frame. A bodyless signal
// may therefore wake an all-optional method without becoming an ABI frame.
static bool flow_node_is_ready(uint32_t node) {
  ...
  if (g_queues[node].empty() && g_pending_bodyless_triggers[node] == 0) return false;
  for (uint32_t r = 0; r < FLOW_REQUIRED_PORT_COUNT; r++) {
...
static int32_t flow_validate_frame_descriptor(
  ...
  if (fd.wire_format == kFlowAlignedBinary) {
    if (fd.type_descriptor_idx >= FLOW_EDGE_COUNT) return -37;
    const FlowEdge &edge = g_edges[fd.type_descriptor_idx];
    if (edge.aligned_eligible == 0 || edge.aligned_byte_length == 0 ||
        edge.aligned_required_alignment == 0 ||
        !flow_is_power_of_two(edge.aligned_required_alignment) ||
        fd.size != edge.aligned_byte_length ||
        fd.alignment < edge.aligned_required_alignment ||
        fd.offset % edge.aligned_required_alignment != 0) {
      return -38;
    }
  }
  return 0;
...
FLOW_EXPORT void space_data_module_runtime_reset_state(void) {
  for (uint32_t n = 0; n < FLOW_NODE_COUNT; n++) {
    g_queues[n].clear();
    g_pending_bodyless_triggers[n] = 0;
...
FLOW_EXPORT int32_t space_data_module_runtime_begin_node_invocation(int32_t node_index,
  ...
  if (count == 0 && g_pending_bodyless_triggers[node] > 0) {
    g_pending_bodyless_triggers[node]--;
  }
...
FLOW_EXPORT int32_t space_data_module_runtime_apply_node_invocation_result(
  ...
  for (int32_t i = 0; i < frame_count; i++) {
    ...
    if (fd.type_descriptor_idx < FLOW_EDGE_COUNT) {
      const FlowEdge &edge = g_edges[fd.type_descriptor_idx];
...
static void flow_enqueue_binding(const FlowTriggerBinding &binding, const char *port,
  ...
  if (descriptor != nullptr) {
    ...
    frame.type_descriptor_idx = descriptor->type_descriptor_idx;
    ...
    frame.ownership = kFlowHostOwned;
    frame.mutability = kFlowImmutable;
    if (descriptor->type_descriptor_idx < FLOW_EDGE_COUNT &&
        descriptor->wire_format == kFlowAlignedBinary) {
      const FlowEdge &edge = g_edges[descriptor->type_descriptor_idx];
      frame.byte_length = edge.aligned_byte_length;
      frame.fixed_string_length = edge.aligned_fixed_string_length;
      frame.required_alignment = edge.aligned_required_alignment;
    }
...
FLOW_EXPORT void space_data_module_runtime_enqueue_trigger_frames(int32_t trigger_index) {
  ...
  for (uint32_t b = 0; b < FLOW_TRIGGER_BINDING_COUNT; b++) {
    ...
    if (flow_binding_requires_frame(g_trigger_bindings[b])) {
      g_ingress_states[trigger_index].total_dropped++;
      continue;
    }
    const uint32_t target = g_trigger_bindings[b].target_node;
    if (g_pending_bodyless_triggers[target] == 0xFFFFFFFFu) {
      g_ingress_states[trigger_index].total_dropped++;
      continue;
    }
    g_pending_bodyless_triggers[target]++;
    accepted++;
    g_node_states[target].ready = flow_node_is_ready(target) ? 1 : 0;
  }
  g_ingress_states[trigger_index].total_received++;
  if (accepted > 0) g_ingress_states[trigger_index].queued_frames++;
}
...
FLOW_EXPORT int32_t space_data_module_runtime_enqueue_trigger_frame(int32_t trigger_index,
  ...
  for (uint32_t b = 0; b < FLOW_TRIGGER_BINDING_COUNT; b++) {
    if (g_trigger_bindings[b].trigger_index != static_cast<uint32_t>(trigger_index)) continue;
    flow_enqueue_binding(g_trigger_bindings[b], port, fd);
    enqueued++;
  }
  g_ingress_states[trigger_index].total_received++;
...
FLOW_EXPORT int32_t space_data_module_runtime_dispatch_current_invocation_direct(
  ...
  for (uint32_t i = 0; i < g_current_desc.frame_count; i++) {
    ...
    if (owned.type_descriptor_idx < FLOW_EDGE_COUNT) {
      const FlowEdge &edge = g_edges[owned.type_descriptor_idx];
      input.schema_name = edge.schema_name;
      input.file_identifier = edge.file_identifier;
      input.root_type_name = edge.root_type_name;
      input.schema_version = edge.schema_version;
      input.schema_hash = edge.schema_hash;
      input.schema_hash_length = edge.schema_hash_size;
    }
