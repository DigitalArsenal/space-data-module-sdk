...
function compareUtf8Bytes(left, right) {
  const a = bundleStatementTextEncoder.encode(String(left));
  const b = bundleStatementTextEncoder.encode(String(right));
...
// Go's encoding/json orders map keys by their UTF-8 bytes. JavaScript's
// localeCompare is locale-aware (and even orders camel-case keys differently),
// while JSON.stringify gives integer-like keys special ordering. Emit the
// small signed statement directly so every SDK host hashes the exact bytes
// used by the production Go verifier.
function goCanonicalJson(value) {
  ...
  if (typeof value === "boolean" || typeof value === "number") {
    if (typeof value === "number" && !Number.isFinite(value)) {
      throw new TypeError(
        "Bundle signature statements cannot contain non-finite numbers.",
      );
    }
    return JSON.stringify(value);
  }
  if (typeof value === "string") return goJsonString(value);
  if (typeof value === "bigint") return goJsonString(value.toString());
  if (Array.isArray(value)) {
    return `[${value
      .map((item) => goCanonicalJson(item) ?? "null")
      .join(",")}]`;
  }
  if (typeof value === "object") {
    ...
    return `{${entries
      .map(([key, nestedValue]) =>
        `${goJsonString(key)}:${goCanonicalJson(nestedValue)}`,
      )
      .join(",")}}`;
  }
  throw new TypeError(
    `Unsupported bundle signature statement value: ${typeof value}`,
  );
}

function canonicalBundleStatementBytes(statement) {
  return bundleStatementTextEncoder.encode(goCanonicalJson(statement));
}
...
export async function computeModuleBundleSignatureHash(bundle, options = {}) {
  ...
  const statement = {
    ...
    entries: entries
      ...
      .sort((left, right) => compareUtf8Bytes(left.entryId, right.entryId)),
  };
